    ...
    <table class="table table-hover responsive nowrap" style="width:100%" id="table">
      ...
      <tbody>
        ...
        <tr>
          <td>{{$rbkd->No}}</td>
          <td>{{$rbkd->Bidang}}</td>
          <td>{{$rbkd->Jenis_Kegiatan}}</td>
          <td>{{$rbkd->Bukti_Penugasan}}</td>
          <td>{{$rbkd->SKS_RBKD}}</td>
          <td>
            <a href="/bkd/pendidikan/{{$rbkd->No}}/edit" class="btn btn-warning btn-sm">Edit</a>
            <a href="/bkd/pendidikan/{{$rbkd->No}}/delete" class="btn btn-danger btn-sm" onclick="return confirm('Yakin Ingin Dihapus ?')">Delete</a>
        </tr>
        @endforeach
      </tbody>
    </table>
